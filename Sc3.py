...
def clean_json_response(response_text):
    """Extract and clean JSON from model output, fixing unquoted keys."""
    ...
    # Fix unquoted keys (e.g., status: -> "status":)
    ...
    json_str = re.sub(r'(?<!\\)"', r'\"', json_str, count=0)  # Escape quotes
    return json_str
...
def process_query(user_query):
    ...
    # Define the system prompt (emphasize strict JSON)
    system_prompt = (
        ...
        "Menu:\n" + context + "\n"
        "Return ONLY valid JSON with all property names enclosed in double quotes. "
        "Do not include any extra text, comments, explanations, or tokens. "
        "Ensure proper JSON syntax, including double quotes around all keys and string values. "
        "Examples:\n"
        "{\"status\": \"success\", \"item\": {\"name\": \"Margherita Pizza\", \"price\": 12.99, \"url\": \"https://example.com/menu/margherita-pizza\"}}\n"
    ...
    outputs = pipe(
        prompt,
        max_new_tokens=100,  # Tight limit to reduce extraneous output
